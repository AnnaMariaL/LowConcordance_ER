...
#Purpose: Calculate robustness of ranking in candidate windows 
rm(list=ls())
...
store_extremes<-function(f_g){
  cmh_pattern<-paste("*_F0F",f_g,"-ordered_CovSampled.sync.cmh",sep = "")
  ...
  print("Read cmh")
  for(rep in c(1:10)){
    fet_pattern<-paste("*F0F",f_g,"-ordered_CovSampled_Rep",rep,".sync.fet",sep="")
