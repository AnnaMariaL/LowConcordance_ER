...
#Purpose: neutral simulations for an empirical p-value distribution of the FET test 
...
for(g.iter in g){
  ...
  for(r.iter in r){
    #grep files
    emp.data<-list.files(pattern = glob2rx(paste("*_F0F",g.iter,"-ordered_CovSampled_Rep",r.iter,".sync.fet",sep="")))
